  ...
  <v-menu v-model="showCart" :close-on-content-click="false" location="bottom">
    ...
    <v-card min-width="300">
      <v-card-item v-if="cart.length === 0">
        <div class="font-weight-normal text-body-1 text-center text-grey-darken-3 mt-2">
          No products selected
        </div>
      </v-card-item>

      <v-list v-for="(productCart, index) in cart" :key="index" v-if="cart.length !== 0">
        <v-list-item :title="productCart.product.name" :subtitle="`Quantity: ${productCart.quantity}`">
          <template v-slot:prepend>
            <v-img :src="productCart.product.images[0]" height="100" width="100"></v-img>
          </template>
          <span class="font-weight-medium">${{ productCart.product.price }}</span>
          <template v-slot:append>
            <v-btn icon="md:delete" variant="text" class="ms-2" color="grey-darken-1"
      ...
      <v-card-item v-if="cart.length !== 0">
        ...
        <div class="font-weight-medium text-h6 text-center text-grey-darken-3 mt-2">Subtotal<span
            class="text-primary font-weight-medium ms-2">${{ subTotal.toFixed(2) }}</span></div>
